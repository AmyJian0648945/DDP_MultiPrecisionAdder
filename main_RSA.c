...
void encryption(uint32_t *rsqModM, uint32_t *rModM, uint32_t e, uint8_t numOfBits, uint32_t *modullus, uint32_t *input_message, uint32_t *output_ciphertext){
	...
	xil_printf("Printing: x_delta \n\r"); print_output(x_delta);
    xil_printf("Printing: A \n\r");       print_output(A);
    xil_printf("Printing: rsqModM - same as A! \n\r"); print_output(rsqModM);
    ...
    for(i = numOfBits; i >= 0; i--){
        ...
        // FOR DEBUGGING:
		xil_printf("Printing: [i=%d] Squaring Result (A) \n\r", i); print_output(A);

        if((e & bin) != 0) {
			mont(A, A, x_delta, modullus);
            xil_printf("Printing: [i=%d] Conditional Multiplication Result (A) \n\r", i); print_output(A);
		}
    ...
    xil_printf("Printing: OUTPUT CIPHERTEXT \n\r"); Print_output(output_ciphertext);
...
void decryption(uint32_t *rsqModM, uint32_t *rModM, uint32_t *d, uint8_t numOfBits, uint32_t *modullus, uint32_t *input_ciphertext, uint32_t *output_message){
    ...
    for(i = numOfBits; i >= 0; i--){
        ...
        for(j = 32; j > 0; j--){
            if(flag == 0){ // makes sure first bit to compute is 1
                if(temp & bin != 0){ 
                    mont(A, A, x_delta, modullus);
                    xil_printf("Printing: [i=%d, j=%d] Squaring Result (A) \n\r", i, j); print_output(A);
                    flag = 1;
                }
            }
    ...
    xil_printf("Printing: OUTPUT MESSAGE \n\r"); Print_output(output_ciphertext);
